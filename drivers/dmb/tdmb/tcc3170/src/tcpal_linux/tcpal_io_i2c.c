...
extern struct i2c_client *dmb_i2c_client;
#endif

static s32 tcpal_blank_func(void)
{
  tcbd_debug(DEBUG_ERROR, "[%s] do nothing!!!\n", __func__);
	return 0;
}

static s32 tcpal_i2c_reg_read(u8 _reg_addr, u8 *_data)
{
  tcc3170_i2c_read_word(I2C_ADDR, _reg_addr, _data);

  //TDMB_MSG_TCC_BB("[%s] addr[0x%x], data[0x%x]\n", __func__, _reg_addr, *_data);
...
{
  //TDMB_MSG_TCC_BB("[%s] addr[0x%x], data[0x%x]\n", __func__, _reg_addr, (unsigned int)_data);

  tcc3170_i2c_write_word(I2C_ADDR, _reg_addr, _data);
  return 0;
}

#if 1
static inline s32 tcpal_i2c_reg_read_burst_tcc(
	u8 _reg_addr, u8 *_data, s32 _size)
{
	...
	for (i = 0; i < num_chunk; i++) {
		sent = i2c_master_send(dmb_i2c_client, &_reg_addr, 1);
		if (sent < 1) {
...
static inline s32 tcpal_i2c_reg_write_burst_tcc(
	u8 _reg_addr, u8 *_data, s32 _size)
{
	s32 ret = 0;
...
#endif
static inline s32 tcpal_i2c_reg_read_burst(u8 _reg_addr, u8 *_data, s32 _size)
{
  s32 i, ret = 0;
  s32 num_chunk, remain;

  if (1)
  {
    ret = tcpal_i2c_reg_read_burst_tcc(_reg_addr, _data, _size);
    return ret;
  }
  ...
  TDMB_MSG_TCC_BB("[%s] addr[0x%x], data[0x%x], size[%d]\n", __func__, _reg_addr, *_data, _size);

	for (i = 0; i < num_chunk; i++)
  {
		tcc3170_i2c_read_len(I2C_ADDR, _reg_addr, &_data[i * MAX_I2C_BURST], MAX_I2C_BURST);

    TDMB_MSG_TCC_BB("[%s] addr[0x%x], data[0x%x], idx[%d]\n", __func__, _reg_addr, (unsigned int)_data[i * MAX_I2C_BURST], i * MAX_I2C_BURST);
	}

	if (remain)
  {
		tcc3170_i2c_read_len(I2C_ADDR, _reg_addr, &_data[num_chunk * MAX_I2C_BURST], remain);

    TDMB_MSG_TCC_BB("[%s] addr[0x%x], data[0x%x], idx[%d]\n", __func__, _reg_addr, (unsigned int)_data[num_chunk * MAX_I2C_BURST], num_chunk * MAX_I2C_BURST);
  }

	return ret;
}

static inline s32 tcpal_i2c_reg_write_burst(u8 _reg_addr, u8 *_data, s32 _size)
{
  s32 i, ret = 0;
  s32 num_chunk, remain;
  ...
  if (1)
  {
    ret = tcpal_i2c_reg_write_burst_tcc(_reg_addr, _data, _size);
    return ret;
  }
	...
	for (i = 0; i < num_chunk; i++)
  {
    TDMB_MSG_TCC_BB("[%s] addr[0x%x], data[0x%x], idx[%d]\n", __func__, _reg_addr, (unsigned int)_data[i * MAX_I2C_BURST], i * MAX_I2C_BURST);
		...
		tcc3170_i2c_write_len(I2C_ADDR, _reg_addr, &_data[i * MAX_I2C_BURST], MAX_I2C_BURST);
	}
	...
	if (remain)
  {
    TDMB_MSG_TCC_BB("[%s] addr[0x%x], data[0x%x], idx[%d]\n", __func__, _reg_addr, (unsigned int)_data[num_chunk * MAX_I2C_BURST], num_chunk * MAX_I2C_BURST);
		...
		tcc3170_i2c_write_len(I2C_ADDR, _reg_addr, &_data[num_chunk * MAX_I2C_BURST], remain);
  }

	return ret;
}

static s32 tcpal_i2c_reg_read_burst_fix(u8 _reg_addr, u8 *_data, s32 _size)
{
	return tcpal_i2c_reg_read_burst(_reg_addr|Bit7, _data, _size);
}

static s32 tcpal_i2c_reg_write_burst_fix(u8 _reg_addr, u8 *_data, s32 _size)
{
	return tcpal_i2c_reg_write_burst(_reg_addr|Bit7, _data, _size);
}

static s32 tcpal_i2c_reg_read_burst_cont(u8 _reg_addr, u8 *_data, s32 _size)
{
	return tcpal_i2c_reg_read_burst(_reg_addr, _data, _size);
}

static s32 tcpal_i2c_reg_write_burst_cont(u8 _reg_addr, u8 *_data, s32 _size)
{
...
void tcpal_set_i2c_io_function(void)
{
	...
	tcbd_i2c_io_funcs->open = tcpal_blank_func;
	tcbd_i2c_io_funcs->close = tcpal_blank_func;
	tcbd_i2c_io_funcs->reg_write = tcpal_i2c_reg_write;
	tcbd_i2c_io_funcs->reg_read  = tcpal_i2c_reg_read;
	tcbd_i2c_io_funcs->reg_write_burst_cont = tcpal_i2c_reg_write_burst_cont;
	tcbd_i2c_io_funcs->reg_read_burst_cont = tcpal_i2c_reg_read_burst_cont;
	tcbd_i2c_io_funcs->reg_write_burst_fix = tcpal_i2c_reg_write_burst_fix;
	tcbd_i2c_io_funcs->reg_read_burst_fix = tcpal_i2c_reg_read_burst_fix;
}
#else
static u8 static_buffer_i2c[MAX_I2C_BURST+4];
