...
#ifdef FEATURE_PANTECH_WLAN_QCOM_PATCH // 20120605, SSR & BT OFF - Kernel panic
...
static DEFINE_SEMAPHORE(hci_smd_enable); // 20120605, SSR & BT OFF - Kernel panic
static int restart_in_progress; //20120525 , SSR - kernel panic issue workaround in BT/WiFi Coex 
#else
...
#ifdef FEATURE_PANTECH_WLAN_QCOM_PATCH 
	down(&hci_smd_enable); // 20120605, SSR & BT OFF - Kernel panic
	restart_in_progress = 1; //20120525 , SSR - kernel panic issue workaround in BT/WiFi Coex 
#else
...
#ifdef FEATURE_PANTECH_WLAN_QCOM_PATCH 
	up(&hci_smd_enable); // 20120605, SSR & BT OFF - Kernel panic
#else
...
#ifdef FEATURE_PANTECH_WLAN_QCOM_PATCH 
	down(&hci_smd_enable); // 20120605, SSR & BT OFF - Kernel panic
	if (restart_in_progress == 1) { //20120525 , SSR - kernel panic issue workaround in BT/WiFi Coex 
...
static int  __init hci_smd_init(void)
{
	...
	wake_lock_init(&hs.wake_lock_tx, WAKE_LOCK_SUSPEND,
			 "msm_smd_Tx");
#ifdef FEATURE_PANTECH_WLAN_QCOM_PATCH 
	restart_in_progress = 0; //20120525 , SSR - kernel panic issue workaround in BT/WiFi Coex 
	hs.hdev = NULL; // 20120605, SSR & BT OFF - Kernel panic
#endif 
	return 0;
}
