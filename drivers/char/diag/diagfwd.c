...
/* Restore Patch : Remove below 2 lines
int diag_event_num_bytes;
int diag_event_config;
*/
struct diag_send_desc_type send = { NULL, NULL, DIAG_STATE_START, 0 };
struct diag_hdlc_dest_type enc = { NULL, NULL, 0 };
/* Restore Patch : Remove mask_info structure
struct mask_info {
	int equip_id;
	int num_items;
	int index;
...
/* Restore Patch : Change APPS_BUF_SIZE to 500 */
...
do {									\
	...
	if (!driver->in_busy_1) {					\
		...
		enc.dest_last = (void *)(driver->buf_in_1 + 499/*APPS_BUF_SIZE - 1*/);\
		...
		diag_device_write(driver->buf_in_1, MODEM_DATA, \
						 driver->write_ptr_1); \
		memset(driver->apps_rsp_buf, '\0', 500/*APPS_BUF_SIZE*/);	\
	}								\
} while (0)
...
/* Restore Patch : Change function parameter num_bytes to num_bits */
static void diag_update_event_mask(uint8_t *buf, int toggle, int num_bits/*num_bytes*/)
{
	uint8_t *ptr = driver->event_masks;
	uint8_t *temp = buf + 2;
	...
	if (!toggle)
		...
		if (CHK_OVERFLOW(ptr, ptr,
				 ptr+EVENT_MASK_SIZE,num_bits/8 + 1/* num_bytes*/))
			memcpy(ptr, temp , num_bits/8+1/*num_bytes*/);
		else
...
{
	uint8_t *temp = buf;
/* Restore Patch : Add mask_info Structure */
	struct mask_info {
		int equip_id;
		int index;
	};
...
/* Restore Patch : Remove below 1 line
...
/* Restore Patch : Remove below function related to mask_update */
#if 0
...
/* Restore Patch : Add belows --> Start */
	/* set event mask */
	if (*buf == 0x82) {
		buf += 4;
...
/* Restore Patch : Remove below Items 
...
*/
/* Restore Patch : Change value 99 to 83 */
		ENCODE_RSP_AND_SEND(83/*99*/);
		return 0;
	}
...
/* Restore Patch : Remove below mask 
...
/* Restore Patch : Remove below mask_update
...
/* Restore Patch : Change APPS_BUF_SIZE to 500 */
		driver->apps_rsp_buf = kzalloc(500/*APPS_BUF_SIZE*/, GFP_KERNEL);
		if (driver->apps_rsp_buf == NULL)
			goto err;
	}
