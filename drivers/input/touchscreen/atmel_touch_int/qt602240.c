...
void disable_autocal_locked(struct work_struct * p) {
	...
	dbg_CR("[#01_PROTECTION] touch auto calibration disable\n");
...
void qt_Acquisition_Config_Init_locked(struct work_struct * p) {
	//mutex_lock(&qt602240_data->lock);
	dbg_CR("[#02_PROTECTION] Finger & Palm recalibration disable\n");
...
uint8_t calibrate_chip(void)
{
	...
	dbg_CR("[#03_TSP] reset acq atchcalst=%d, atchcalsthr=%d\n", acquisition_config.atchcalst, acquisition_config.atchcalsthr );  // mirinae_test
...
{    
	...
	if (good_calibration_cnt >= 3) { // p11223_20120430   // 1 ==> 3
		...
		if ((jiffies_to_msecs(jiffies) - mxt_time_point) >= 300) {
			...
			acquisition_config.atchfrccalthr = T8_ATCHFRCCALTHR;     /*!< Anti-touch force calibration threshold */
			acquisition_config.atchfrccalratio = T8_ATCHFRCCALRATIO;  /*!< Anti-touch force calibration ratio */  
			dbg("[#07_TSP] write acq atchcalst=%d, atchcalsthr=%d\n", acquisition_config.atchcalst, acquisition_config.atchcalsthr );
			if (write_acquisition_config(acquisition_config) != CFG_WRITE_OK){
				printk("\n[TSP][ERROR] line : %d\n", __LINE__);
			}

			dbg_CR("[#08_TSP] T6 Calibration success!! \n");
		}
		else { 
			cal_check_flag = 1u;
			good_calibration_cnt++;
...
int8_t check_chip_calibration(void)
{
	...
	if(get_touch_antitouch_info()) {
		...
		// Count fingers
		for (i = 0 ; i < MAX_NUM_FINGER; ++i) { 
			if (fingerInfo[i].status <= 0) // p11223_20120430
		...
		// Else. 
			...
			if(((not_yet_count >= 4) &&(finger_cnt== 0)) ||(not_yet_count >= 10) )  {		 // p11223_20120430
...
int8_t get_touch_antitouch_info(void) {
	...
	/* data is ready - read the detection flags */
	read_mem(diag_address, 100,data_buffer);  // p11223_20120509
	...
	if((data_buffer[0] == 0xF3) && (data_buffer[1] == 0x00))
	{
		...
		{
			/* print the flags to the log - only really needed for debugging */
			dbg("[TSP] Detect Flags X%d, %x%x, %x%x \n", i>>1,data_buffer[3+i],data_buffer[2+i],data_buffer[47+i],data_buffer[46+i]);    // p11223_20120509
			...
			for(j = 0; j < 8; j++)
			{
				...
				if(data_buffer[46+i] & check_mask){   // p11223_20120509
					atch_ch++;
				}
				if(data_buffer[47+i] & check_mask){   // p11223_20120509
					atch_ch++;
				}
			}
		}

#if 0  // p11223_20120509 for Test
		for(i=0; i<10;i++)
		{
			for(j=0; j<10;j++)
				dbg("  %x ", data_buffer[i*10+j]);
				dbg("\n ");
		}
#endif
...
void  get_message(struct work_struct * p)
{
	...
	if (object_type == TOUCH_MULTITOUCHSCREEN_T9)
	{
		...
		{
			if(f_resume){
				dbg_CR("[#19_TOUCH] TOUCH Release event but,debugInfo.autocal_flag is true id => %d \n",id);
				goto interrupt_return;
			}
			...
			touch_message_flag = 1;
			dbg_CR("#20_##### Finger[%d] Up (%d,%d)\n", id, fingerInfo[id].x, fingerInfo[id].y );
		}
		else if ( (quantum_msg[1] & TOUCH_DETECT) && (quantum_msg[1] & TOUCH_MOVE) )  
		{
			if(f_resume){
				dbg_CR("[#21_TOUCH] TOUCH Move event but,debugInfo.autocal_flag is true id => %d \n",id);
				goto interrupt_return;
			}
			fingerInfo[id].id= id;
		...
		{   
			...
			if(debugInfo.autocal_flag==1){																			// After touch calibration, first touch press event.
				dbg_CR("[#20_2PROTECTION] First touch event after touch calibration\n");
				debugInfo.autocal_flag++;
				if(f_resume_cal){
								dbg_CR("[#20_3PROTECTION] f_resume_cal = %d\n", f_resume_cal);
		...
		dbg("[#22_TSP] msg id =  %d, status = 0x%x \n", quantum_msg[0],quantum_msg[1]);
		if((quantum_msg [1] != 0x10) && (quantum_msg [1] != 0x00))
			goto interrupt_return;

#ifdef PROTECTION_MODE 			
		if(quantum_msg[1] == 0x10) {
			dbg_CR("[#23_PROTECTION] T6 Calibrating!\n");
			cal_check_flag=1u;
			mxt_time_point = 0;
#ifdef ANTI_TOUCH_RECOVERY_AFTER_PROTECTION_MODE
			f_delta_check=false;
            ...
            if(debugInfo.autocal_flag){                                     // If touch auto calibration is enabled.
                dbg_CR("[#24_PROTECTION] Touch calibration is executed in auto calibration\n");
				cancel_work_sync(&qt602240_data->work_tchautocal_disable);
				del_timer(&tchautocal_timer);
			}else{
				dbg_CR("[#25_PROTECTION] First Touch & Auto calibration is executed \n");
			...
			if(!f_resume_cal){
								dbg_CR("[#25_1PROTECTION] f_resume_cal = %d\n", f_resume_cal);
...
void report_input (int touch_status) {
	...
	if(!touch_status){
		...
		if (valid_input_count == 0 && cal_correction_limit > 0) {
			...
			dbg_CR("[#30_PROTECTION] Finger & Palm recalibration start 300ms [%d]\n",cal_correction_limit);
			acquisition_config.atchcalst = T8_ATCHCALST_PROTECTION;
			acquisition_config.atchcalsthr = T8_ATCHCALSTHR_PROTECTION; // Never suspend
			acquisition_config.atchfrccalthr = T8_ATCHFRCCALTHR_PROTECTION;
			acquisition_config.atchfrccalratio = T8_ATCHFRCCALRATIO_PROTECTION;
