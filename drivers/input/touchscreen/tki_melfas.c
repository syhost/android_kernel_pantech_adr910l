...
/* GPIO Setting */
/* -------------------------------------------------------------------- */
#if ((defined(CONFIG_MACH_MSM8960_CHEETAH)) && (BOARD_VER > WS10))  // mirinae_p11223
#define GPIO_TKI_SDA			95 
#define GPIO_TKI_SCL			96
#elif ((defined(CONFIG_MACH_MSM8960_CHEETAH)) && (BOARD_VER <= WS10))
#define GPIO_TKI_SDA			40 
#define GPIO_TKI_SCL			41
#endif
#define GPIO_TKI_CHG			68
#define IRQ_TKI_INT			gpio_to_irq(GPIO_TKI_CHG)
#define GPIO_TKI_POWER 			90    // mirinae test for cheetah WS13 Board
#define GPIO_MAIN_TOUCH_POWER 	51    
...
#define TKI_REG_WRITE_COMMAND		0x10  // mirinae_tm
...
{
	int rc; 
	struct regulator *vreg_power = NULL;
#if (BOARD_VER > WS12)
	unsigned gpioConfig;  //mirinae_ws13
#endif
	
	dbg_func_in();

	disable_irq(tki_data->client->irq);  // added

	if(BOARD_VER > WS12){   //mirinae_ws13
  		gpio_set_value(GPIO_TKI_POWER, 0);
		dbg("(skytouch)set GPIO_TKI_POWER Low.\n");
		//msleep(10);
	}
	
#if defined(CONFIG_MACH_MSM8960_CHEETAH) 
 #if (BOARD_VER== WS10)  	
	vreg_power = regulator_get(NULL, "8921_l15");
 #else
	vreg_power = regulator_get(NULL, "8921_l17");
 #endif
#endif
...
////////////////////////// added
#if (BOARD_VER > WS12)
	gpio_request(GPIO_TKI_POWER, "touch_power_n");
	gpioConfig = GPIO_CFG(GPIO_TKI_POWER, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA);
	...
	gpio_set_value(GPIO_TKI_POWER, 0);
	dbg("(skytouch)set GPIO_TKI_POWER Low  tki_early_suspend.\n");
#endif
//////////////////////////
	dbg_func_out();
...
static int  tki_late_resume(struct early_suspend *h)
{
...
#if defined(CONFIG_MACH_MSM8960_CHEETAH) // mirinae_20110920
	...
	if(BOARD_VER > WS12){   //mirinae_ws13
  		gpio_set_value(GPIO_TKI_POWER, 1);
		dbg("(skytouch)set GPIO_TKI_POWER High.\n");
		//msleep(10);
	}
	enable_irq(tki_data	->client->irq);  // added
...
#if (BOARD_VER <= WS12)    
  	struct regulator  *vreg_touch_LVS4;
#endif	
...
//  1. INPUT POWER --> GPIO51 
	gpio_request(GPIO_MAIN_TOUCH_POWER, "touch_power_n");
	gpioConfig = GPIO_CFG(GPIO_MAIN_TOUCH_POWER, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA);
	rc = gpio_tlmm_config(gpioConfig, GPIO_CFG_ENABLE);
	if (rc) {
		printk(KERN_ERR "%s: GPIO_TOUCH_RST failed (%d)\n",__func__, rc);
		return -1;
	}
	gpio_set_value(GPIO_MAIN_TOUCH_POWER, 1);
	dbg("(skytouch)set GPIO_MAIN_TOUCH_POWER High.\n");

// 2. Init 3.3V regulator --> L17
	vreg_power = regulator_get(NULL, "8921_l17");
	...
	rc = regulator_enable(vreg_power);
	printk("[TKI] Touch Power regulator_enable return:  %d \n", rc);
	regulator_put(vreg_power);

//  3. Level Shifter Enable --> GPIO90 
	gpio_request(GPIO_TKI_POWER, "tki_power_n");
	gpioConfig = GPIO_CFG(GPIO_TKI_POWER, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA);
	rc = gpio_tlmm_config(gpioConfig, GPIO_CFG_ENABLE);
	if (rc) {
		printk(KERN_ERR "%s: GPIO_TKI_POWER failed (%d)\n",__func__, rc);
		return -1;
	}
	gpio_set_value(GPIO_TKI_POWER, 1);
	dbg("(tki_power)set GPIO_TKI_POWER High.\n");
  #elif (BOARD_VER <= WS12)
...
#ifdef CURRENT_OPTIMIZATION
	gpioConfig = GPIO_CFG(GPIO_TKI_CHG, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA);
#else
...
#ifdef CURRENT_OPTIMIZATION
	rc = gpio_tlmm_config(GPIO_CFG(GPIO_TKI_SDA, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc) {
		pr_err("%s: Could not configure gpio tki_sda\n", __func__);
		return rc;
	}
	rc = gpio_tlmm_config(GPIO_CFG(GPIO_TKI_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc) {
		pr_err("%s: Could not configure gpio tki_scl\n", __func__);
		return rc;
	}
#endif
