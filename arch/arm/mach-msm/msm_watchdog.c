...
#define DLOAD_MAGIC_WDT_ADR       		0x8D300000
#define DLOAD_MAGIC_WDT				0x95857493
#endif
...
#ifdef CONFIG_PANTECH_ERR_CRASH_LOGGING
        if(appsbark == 1)
            __raw_writel(3, msm_tmr0_base + WDT0_EN);
        else
...
static irqreturn_t wdog_bark_handler(int irq, void *dev_id)
{
	...
	if(appsbark == 1)
	{
		wdt_addr = (unsigned long) DLOAD_MAGIC_WDT_ADR;
		wdt_magic_addr = (void*)ioremap_nocache(wdt_addr, SZ_4K);
		__raw_writel(DLOAD_MAGIC_WDT, wdt_magic_addr);
	}
#endif
...
static void init_watchdog_work(struct work_struct *work)
{
	...
	if(appsbark == 1)
	{
		wdt_addr = (unsigned long) DLOAD_MAGIC_WDT_ADR;
		wdt_magic_addr = (void*)ioremap_nocache(wdt_addr, SZ_4K);
		__raw_writel(0xffffffff, wdt_magic_addr);
	}
#endif
