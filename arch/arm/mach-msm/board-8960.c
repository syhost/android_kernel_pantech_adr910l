...
#ifdef T_STARQ  // ddr_768M_config
unsigned int pantech_ddr_768M_check(void);
#endif 
...
#ifdef T_STARQ // ddr_768M_config
#define MSM8960_FIXED_AREA_START_768M 0xa0000000
#define MSM_MM_FW_SIZE_768M		0x200000
#define MSM8960_FW_START_768M	(MSM8960_FIXED_AREA_START_768M - MSM_MM_FW_SIZE_768M)
#endif
...
#ifdef T_STARQ  // ddr_768M_config
    if(pantech_ddr_768M_check())
	    reserve_info->fixed_area_start = MSM8960_FW_START_768M;
	else
...
#ifdef T_STARQ // ddr_768M_config
    if(pantech_ddr_768M_check())
	    fixed_size = (fixed_size + MSM_MM_FW_SIZE_768M + SECTION_SIZE - 1) & SECTION_MASK;
	else
	    fixed_size = (fixed_size + MSM_MM_FW_SIZE + SECTION_SIZE - 1) & SECTION_MASK;
#else	 
	fixed_size = (fixed_size + MSM_MM_FW_SIZE + SECTION_SIZE - 1) & SECTION_MASK;
#endif		
	msm8960_reserve_fixed_area(fixed_size);
#else  // #if 0 // 20120608 jylee
    if(pantech_ddr_768M_check()) {
	    fixed_size = (fixed_size + MSM_MM_FW_SIZE_768M + SECTION_SIZE - 1) & SECTION_MASK;
		msm8960_reserve_fixed_area(fixed_size);
	} else
	    msm8960_reserve_fixed_area(fixed_size + MSM_MM_FW_SIZE);
#endif  // #if 0 // 20120608 jylee

#ifdef T_STARQ // ddr_768M_config
    if(pantech_ddr_768M_check())
	    fixed_low_start = MSM8960_FIXED_AREA_START_768M;
	else
...
#ifdef T_STARQ // ddr_768M_config
static void __init locate_unstable_memory_768M(void)
{
	...
	if (high < MAX_FIXED_AREA_SIZE + MSM8960_FIXED_AREA_START_768M)
...
#ifdef T_STARQ //ddr_768M_config
        if(pantech_ddr_768M_check()) {
		    fmem_pdata.phys = reserve_info->fixed_area_start + MSM_MM_FW_SIZE_768M;
		    pr_info("mm fw at %lx (fixed) size %x\n", reserve_info->fixed_area_start, MSM_MM_FW_SIZE_768M);
		} 
		else {
		    fmem_pdata.phys = reserve_info->fixed_area_start + MSM_MM_FW_SIZE;
